...
# /broadcast
BROADCASTING_TEXT = '<i>Отправка...</i>'
WARNING_TEXT = '<b>⚠ Внимание!</b>\n'
...
async def broadcast(message: types.Message):
    if message.from_user.id in config.admins:
        if message.text != '/broadcast':
            await message.answer(BROADCASTING_TEXT, parse_mode='HTML')
            chats_list = os.listdir('Bases/')
            chats_list.remove('botbase.txt')
            text = message.text.split(' ', 1)[1]
            for chat in chats_list:
                try:
                    await bot.send_message(int(chat[:-4]), f'{WARNING_TEXT}{text}', parse_mode='HTML')
                except:
                    pass
        else:
            pass
...
async def send_admin(message: types.Message):
    if message.chat.id != message.from_user.id:
        ...
        try:
            ...
            await bot.send_message(config.admin_id, ADMIN_TEMPLATE % {"id": message.from_user.id, "username": message.from_user.first_name, "msg": args[1]}, parse_mode='HTML')
            await message.reply(SEND_TXT_ADMIN, parse_mode="HTML")
            await bot.send_message(config.logs_channel_id, ADMIN_LOGS_TEMPLATE % {"id": message.from_user.id, "username": message.from_user.first_name, "msg": args[1]}, parse_mode='HTML')
        except IndexError:
            await message.reply(SEND_HLP_ADMIN, parse_mode='HTML')

@dp.message(Command(commands='resend'))
async def resend_admin(message: types.Message):
    if message.from_user.id in config.admins:
        if not len(message.text.split(' ', 1)[1]) == 0:
            try:
                args = message.text.split(' ', 2)
                await bot.send_message(config.logs_channel_id, RESENDED_BY_MESSAGE % {"id": message.from_user.id, "msg": args[2]}, parse_mode='HTML')
